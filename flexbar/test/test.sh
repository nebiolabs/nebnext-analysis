...
$cmd = qq{flexbar --reads $ENV{obs_dir}/$val{library}.end_left.adp_tso.new_1.fastq --reads2 $ENV{obs_dir}/$val{library}.end_left.adp_tso.new_2.fastq --target $ENV{obs_dir}/$val{library} --adapters $ENV{adapter_dir}/ilmn_20.tso.new.fasta --adapter-trim-end RIGHT --adapter-revcomp ALSO --threads $ENV{num_threads} --align-log ALL};
print {*STDERR} "$cmd";
system $cmd;
