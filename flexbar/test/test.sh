...
$cmd = qq{flexbar --reads $ENV{fastq_dir}/$val{library}_1.fastq --reads2 $ENV{fastq_dir}/$val{library}_2.fastq --target $ENV{obs_dir}/$val{library}.tso_wo_hp_left_right_revcomp_also --adapters $ENV{adapter_dir}/tso_wo_hp.fasta --adapter-trim-end LEFT --adapter-revcomp ALSO --adapter-revcomp-end RIGHT --htrim-left GT --htrim-right CA --htrim-min-length 3 --htrim-max-length 5 --htrim-max-first --htrim-adapter --threads $ENV{num_threads} --align-log ALL};
print {*STDERR} "$cmd";
system $cmd;

$cmd = qq{flexbar --reads $ENV{obs_dir}/$val{library}.tso_wo_hp_left_right_revcomp_also_1.fastq --reads2 $ENV{obs_dir}/$val{library}.tso_wo_hp_left_right_revcomp_also_2.fastq --target $ENV{obs_dir}/$val{library} --adapters $ENV{adapter_dir}/ilmn_20.fasta --adapter-trim-end RIGHT --threads $ENV{num_threads} --align-log ALL};
print {*STDERR} "$cmd";
system $cmd;
